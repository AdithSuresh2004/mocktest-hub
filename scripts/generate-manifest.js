...
function extractExamMetadata(filePath) {
  try {
    ...
    const examType =
      exam.exam_type || exam.type || detectExamType(exam, pathParts)
    const strength =
      exam.difficulty || exam.strength || exam.exam_strength || 'medium'
    ...
    return {
      ...
      description:
        exam.description || `${exam.name || exam.exam_name || fileName} exam`,
      tags: exam.tags || [],
    }
  } catch (error) {
...
function generateManifest() {
  ...
  for (const filePath of allJsonFiles) {
    ...
    if (exam) {
      ...
      if (examsByType[typeKey]) {
        ...
        console.warn(`  ✓ ${exam.name} [${typeKey}] (${exam.category})`)
      }
    }
  }
  ...
  const totalExams =
    manifest.full_tests.length +
    manifest.subject_tests.length +
    manifest.topic_tests.length
  try {
