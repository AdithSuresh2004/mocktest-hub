...
async function main() {
  // Allow forcing regeneration in a few ways:
  // - via npm script name: `npm run manifest:regen` (npm sets npm_lifecycle_event)
  // - via CLI flags: --regen or --force
  // - via env var: FORCE_MANIFEST=1
  const lifecycle = process.env.npm_lifecycle_event || ''
  const cliForce = process.argv.includes('--regen') || process.argv.includes('--force')
  const envForce = process.env.FORCE_MANIFEST === '1'
  const forceRun = lifecycle === 'manifest:regen' || cliForce || envForce

  if (fs.existsSync(sentinel) && !forceRun) {
