...
const calculateAnalysis = (examData, attemptData) => {
    ...
    const sectionScores = typeof attemptData.score === 'object' && attemptData.score?.per_section
    ...
    const sectionAnalysis = examData.sections.map((section, sectionIndex) => {
        ...
        const sectionKey = section.section_id || section.section_name?.[0] || String.fromCharCode(65 + sectionIndex);
        ...
        const actualSectionMarks = sectionScores[sectionKey] !== undefined
        ...
        return {
            ...
            accuracy: section.questions.length > 0
    ...
    return {
        ...
        overall: {
            ...
            accuracy: totalAttempted > 0
                ...
                : 0,
            percentage: totalQuestionsOverall > 0
...
export const getPerformanceLevel = (percentage) => {
    ...
    if (numPercentage >= 90) return { text: "Excellent", color: "text-green-600 dark:text-green-400" };
    if (numPercentage >= 75) return { text: "Very Good", color: "text-blue-600 dark:text-blue-400" };
    if (numPercentage >= 60) return { text: "Good", color: "text-yellow-600 dark:text-yellow-400" };
    if (numPercentage >= 40) return { text: "Average", color: "text-orange-600 dark:text-orange-400" };
