  ...
  const allTests = useMemo(
    () => [
      ...(manifest.full_tests || []).map((t, index) => ({ 
        ...t, 
        type: 'full_tests',
        exam_id: t.id || t.exam_id, // Normalize id field
        exam_name: t.name || t.exam_name, // Normalize name field
        exam_strength: t.difficulty || t.exam_strength, // Normalize difficulty field
        uid: `full_tests-${t.id || t.exam_id || index}`, // Unique ID for React keys
      })),
    ...
    ].filter(t => t.exam_id && t.exam_name), // Filter out invalid tests
    [manifest],
  )
  ...
  const filteredTests = useMemo(() => {
    return allTests
      ...
      .filter((test) => {
        ...
        return test.exam_strength?.toLowerCase() === selectedStrength.toLowerCase()
      })
      .filter((test) => {
