...
function Toast({ toast, onClose }) {
  const { icon: Icon, className: iconClass } = iconMap[toast.type] || iconMap.info
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      <Icon className={`h-5 w-5 flex-shrink-0 ${iconClass}`} aria-hidden="true" />
      <p className="flex-1 text-sm font-medium text-gray-900 dark:text-gray-100">
        {toast.message}
      </p>
      <button
        onClick={onClose}
        className="flex-shrink-0 rounded-md text-gray-400 transition-colors hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 dark:hover:text-gray-300"
