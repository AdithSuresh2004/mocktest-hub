...
export const filterTests = (tests, filters) => {
  return tests.filter((test) => {
    ...
    if (filters.selectedExam && filters.selectedExam !== 'All Exams') {
      if (test.category?.toLowerCase() !== filters.selectedExam.toLowerCase()) return false
    }
    
    if (filters.selectedTopic && filters.selectedTopic !== 'All Topics') {
      if (test.topic !== filters.selectedTopic && 
    ...
    if (filters.selectedSubject && filters.selectedSubject !== 'All Subjects') {
      if (test.subject !== filters.selectedSubject && 
    ...
    if (filters.selectedStrength && filters.selectedStrength !== 'All levels') {
      if (test.exam_strength?.toLowerCase() !== filters.selectedStrength.toLowerCase()) return false
    }
    
    if (filters.selectedAttemptStatus && filters.selectedAttemptStatus !== 'all') {
      const hasAttempted = filters.attemptedExams?.has(test.exam_id)
      if (filters.selectedAttemptStatus === 'attempted' && !hasAttempted) return false
      if (filters.selectedAttemptStatus === 'unattempted' && hasAttempted) return false
    }
